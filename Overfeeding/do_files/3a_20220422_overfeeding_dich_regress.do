...
gen overfeeding = 1
replace overfeeding = 2 if condition == "A"
replace overfeeding = 2 if condition == "D"
tab overfeeding condition, missing

gen diet = 1
replace diet = 2 if condition == "A"
replace diet = 2 if condition == "B"
...
* now append
clear
save "RESULTS/overfeeding_batch/overfeeding_batch.dta", replace emptyok 
...
gen model="`file'"
append using "RESULTS/overfeeding_batch/overfeeding_batch.dta"
save "RESULTS/overfeeding_batch/overfeeding_batch.dta", replace
}
use "RESULTS/overfeeding_batch/overfeeding_batch.dta", clear
...
save "RESULTS/fixed_effects_overfeeding_batch.dta", replace
export excel using "RESULTS/fixed_effects_overfeeding_batch.xls", replace firstrow(variables)
...
* now append
clear
save "RESULTS/overfeeding_size/overfeeding_size.dta", replace emptyok 
...
gen model="`file'"
append using "RESULTS/overfeeding_size/overfeeding_size.dta"
save "RESULTS/overfeeding_size/overfeeding_size.dta", replace
}
use "RESULTS/overfeeding_size/overfeeding_size.dta", clear
drop if strpos(parm,"b.")
drop if strpos(parm,"o.")
drop if eq == "lnalpha"
replace t = z if t == .
...
drop if strpos(parm,"batch")
drop if strpos(parm,"tank")
save "RESULTS/fixed_effects_overfeeding_size.dta", replace
export excel using "RESULTS/fixed_effects_overfeeding_size.xls", replace firstrow(variables)
...
use "RESULTS/fixed_effects_overfeeding_batch.dta", clear
merge 1:1 trait model parm using "RESULTS/fixed_effects_overfeeding_size.dta"
drop _merge
...
replace trait_order = 55 if strpos(trait,"AreaEye")
...
replace parm = "overfeeding" if parm == "2.overfeeding"

outsheet using "RESULTS/overfeeding_RegardlessOfDied_results.txt", noquote replace
