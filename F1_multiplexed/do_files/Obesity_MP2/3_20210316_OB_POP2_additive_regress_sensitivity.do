...
preserve
nbreg `outcome' sh2b1_D bdnf_D irs1_D irs2a_D irs2b_D sim1a_D sim1b_D tod i.batch
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS/exposure_pop2_add_un_sensitivity/`outcome'_est", replace)
use "RESULTS/exposure_pop2_add_un_sensitivity/`outcome'_est", clear
gen N=e(N)
save "RESULTS/exposure_pop2_add_un_sensitivity/`outcome'_est", replace
restore
}
...
preserve
logit `outcome' sh2b1_D bdnf_D irs1_D irs2a_D irs2b_D sim1a_D sim1b_D tod i.batch
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS/exposure_pop2_add_un_sensitivity/`outcome'_est", replace)
use "RESULTS/exposure_pop2_add_un_sensitivity/`outcome'_est", clear
gen N=e(N)
save "RESULTS/exposure_pop2_add_un_sensitivity/`outcome'_est", replace
...
* now append
clear
save "RESULTS/exposure_pop2_add_un_sensitivity/exposure_pop2_add_un_sensitivity.dta", replace emptyok 

foreach file in bld_est bodyfat_est {

use "RESULTS/exposure_pop2_add_un_sensitivity/`file'", clear
gen model="`file'"
append using "RESULTS/exposure_pop2_add_un_sensitivity/exposure_pop2_add_un_sensitivity.dta"
save "RESULTS/exposure_pop2_add_un_sensitivity/exposure_pop2_add_un_sensitivity.dta", replace
}
use "RESULTS/exposure_pop2_add_un_sensitivity/exposure_pop2_add_un_sensitivity.dta", clear
...
drop id order eq
save "RESULTS/fixed_effects_ob_pop2_add_un_sensitivity.dta", replace
export excel using "RESULTS/fixed_effects_ob_pop2_add_un_sensitivity.xls", replace firstrow(variables)
...
preserve
regress `outcome' sh2b1_D bdnf_D irs1_D irs2a_D irs2b_D sim1a_D sim1b_D tod i.batch i.tank
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS/exposure_pop2_add_batch_sensitivity/`outcome'_est", replace)
use "RESULTS/exposure_pop2_add_batch_sensitivity/`outcome'_est", clear
gen N=e(N)
save "RESULTS/exposure_pop2_add_batch_sensitivity/`outcome'_est", replace
restore
}
...
* now append
clear
save "RESULTS/exposure_pop2_add_batch_sensitivity/exposure_pop2_add_batch_sensitivity.dta", replace emptyok 
...
use "RESULTS/exposure_pop2_add_batch_sensitivity/`file'", clear
gen model="`file'"
append using "RESULTS/exposure_pop2_add_batch_sensitivity/exposure_pop2_add_batch_sensitivity.dta"
save "RESULTS/exposure_pop2_add_batch_sensitivity/exposure_pop2_add_batch_sensitivity.dta", replace
}
use "RESULTS/exposure_pop2_add_batch_sensitivity/exposure_pop2_add_batch_sensitivity.dta", clear
drop if strpos(parm,"b.")
drop if strpos(parm,"o.")
drop if eq == "lnalpha"
replace t = z if t == .
...
sort trait model order
drop id order eq
save "RESULTS/fixed_effects_ob_pop2_add_batch_sensitivity.dta", replace
export excel using "RESULTS/fixed_effects_ob_pop2_add_batch_sensitivity.xls", replace firstrow(variables)
...
** Examine the effect of OB POP2 genes additionally adjusted for size
use "DATA/Obesity_POP2/OB_POP2_for_analysis_sensitivity.dta", clear
set more off
...
preserve
regress `outcome' sh2b1_D bdnf_D irs1_D irs2a_D irs2b_D sim1a_D sim1b_D tod length dorsal_area i.batch i.tank
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", replace)
use "RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", clear
gen N=e(N)
save "RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", replace
restore
}
...
preserve
nbreg `outcome' sh2b1_D bdnf_D irs1_D irs2a_D irs2b_D sim1a_D sim1b_D tod i.batch length dorsal_area
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", replace)
use "RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", clear
gen N=e(N)
save "RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", replace
...
preserve
logit `outcome' sh2b1_D bdnf_D irs1_D irs2a_D irs2b_D sim1a_D sim1b_D tod i.batch length dorsal_area
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", replace)
use "RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", clear
gen N=e(N)
save "RESULTS/exposure_pop2_add_size_sensitivity/`outcome'_est", replace
...
* now append
clear
save "RESULTS/exposure_pop2_add_size_sensitivity/exposure_pop2_add_size_sensitivity.dta", replace emptyok 
...
use "RESULTS/exposure_pop2_add_size_sensitivity/`file'", clear
gen model="`file'"
append using "RESULTS/exposure_pop2_add_size_sensitivity/exposure_pop2_add_size_sensitivity.dta"
save "RESULTS/exposure_pop2_add_size_sensitivity/exposure_pop2_add_size_sensitivity.dta", replace
}
use "RESULTS/exposure_pop2_add_size_sensitivity/exposure_pop2_add_size_sensitivity.dta", clear
drop if strpos(parm,"b.")
drop if strpos(parm,"o.")
drop if eq == "lnalpha"
replace t = z if t == .
...
sort trait model order
drop id order eq
save "RESULTS/fixed_effects_ob_pop2_add_size_sensitivity.dta", replace
export excel using "RESULTS/fixed_effects_ob_pop2_add_size_sensitivity.xls", replace firstrow(variables)
...
use "RESULTS/fixed_effects_ob_pop2_add_un_sensitivity.dta", clear
merge 1:1 trait model parm using "RESULTS/fixed_effects_ob_pop2_add_batch_sensitivity.dta"
drop _merge
merge 1:1 trait model parm using "RESULTS/fixed_effects_ob_pop2_add_size_sensitivity.dta"
drop _merge

gen order = .
replace order = 1 if strpos(parm,"sh2b1_D")
replace order = 2 if strpos(parm,"bdnf_D")
replace order = 3 if strpos(parm,"irs1_D")
replace order = 4 if strpos(parm,"irs2a_D")
replace order = 5 if strpos(parm,"irs2b_D")
replace order = 6 if strpos(parm,"sim1a_D")
replace order = 7 if strpos(parm,"sim1b_D")
...
outsheet using "RESULTS/OB2_sensitivity/obesity_pop2_add_linear_results_sensitivity.txt", noquote replace
